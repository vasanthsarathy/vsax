...
class TestFHRROperations:
    ...
    def test_inverse_frequency_domain_conjugate(self, ops):
        """Test that inverse uses frequency-domain conjugate.

        This validates the mathematical correctness of the fix.
        """
        import jax
        ...
        # Compute inverse using FHRROperations
        inv_a = ops.inverse(a)

        # Manual computation: ifft(conj(fft(a)))
        expected = jnp.fft.ifft(jnp.conj(jnp.fft.fft(a)))

        assert jnp.allclose(inv_a, expected, atol=1e-6), (
            "Inverse must use frequency-domain conjugate: ifft(conj(fft(a)))"
        )
