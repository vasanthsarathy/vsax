...
\begin{itemize}
    \item Non-commutative operator algebras for richer compositional structures
    \item Learned operator parameters optimized for specific reasoning tasks
    \item Extended operator support for MAP and Binary representations
    \item Probabilistic VSA extensions for uncertainty quantification
    \item Hardware-specific backends (TPU, neuromorphic chips)
    \item Temporal binding operations for sequence modeling
\end{itemize}
...
\begin{itemize}
    \item JAX: v0.4.23 with CUDA 12.0 support
    \item NumPy: v1.24.3
    \item Python: 3.9.18
    \item CUDA: 12.0
    \item cuDNN: 8.9.0
...
\begin{itemize}
    \item VSA model initialization: \texttt{jax.random.PRNGKey(42)}
    \item Encoder initialization: \texttt{jax.random.PRNGKey(100)}
    \item Operator generation: Fixed seeds per operator (1000-1015 for spatial, 2000-2015 for semantic)
\end{itemize}

\paragraph{Default Hyperparameters}
Unless otherwise specified:
\begin{itemize}
    \item Dimensionality: 1024 (FHRR, MAP), 10000 (Binary)
    \item Resonator iterations: 50
    \item Resonator convergence threshold: $10^{-6}$
    \item Similarity threshold (cleanup): 0.3
\end{itemize}
...
\begin{itemize}
    \item Similarity: Cosine similarity $\in [-1, 1]$
    \item Inversion accuracy: $\cos(\mathcal{O}^{-1}(\mathcal{O}(v)), v) > 0.999$
    \item Cleanup accuracy: Fraction of queries returning correct symbol (similarity $>$ 0.3)
\end{itemize}

\subsection{Code Availability}

All benchmark scripts, example code, and experiment configurations are available in the VSAX repository:
\begin{itemize}
    \item Benchmarks: \texttt{examples/benchmarks/}
    \item Use cases: \texttt{examples/operators/}, \texttt{examples/resonators/}
    \item Tests: \texttt{tests/} (450 tests, 95\% coverage)
\end{itemize}

\subsection{Data Availability}

All datasets used in use case demonstrations are publicly available:
\begin{itemize}
    \item MNIST: \url{http://yann.lecun.com/exdb/mnist/}
    \item WordNet: \url{https://wordnet.princeton.edu/}
    \item Example knowledge graphs: Included in VSAX repository
\end{itemize}
